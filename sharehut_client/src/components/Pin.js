...
const Pin = ({pin:{_id, image, posted, destination,save}}) => {
    ...
    <div className='absolute top-0 w-full h-full flex flex-col justify-between p-1 pr-2 pt-2 pb-2 z-50' style={{height:'100%'}} >
        <div className='flex items-center justify-between'>
            <div className='flex gap-2'>
                ...
                className="bg-white w-9 h-9 rounded-full flex items-center justify-center text-dark text-xl opacity-8 5 hover:opacity-100 hover:shadow-md outline-none  "
                >
                    <FcDownload/>
                </a>
            </div>
            ...
            {alreadySaved? 
                <button type="button" disabled className='cursor-pointer  bg-orange-500 opacity-70 hover:opacity-100 text-white font-bold px-5 py-0  text-base rounded-3xl hover:shadow-md outlined-none'> {save?.length} Saved</button>
                :
